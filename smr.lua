...
Converted["_ImageLabel"].Image = "rbxassetid://74603139524347"
...
Converted["_TextLabel7"].Text = "SMR"
...
local function BYVMH_routine() -- Script: StarterGui.YARHM.Universal
	...
	--table.insert(module, {
	--	Type = "Text",
	--	Args = {"Welcome to SMR! The open, free script hub."}
	--})
	...
	table.insert(module, {
		...
		Args = {"Join our Discord", function(Self)
			if setclipboard then setclipboard("https://discord.gg/AduQ7cq427") end
	...
	table.insert(module, {
		...
		Args = {"Hide SMR", function(Self)
			...
			elseif uis.AccelerometerEnabled then
				...
				fu.notification("Can't hide SMR!") -- how else are you gonna open???
			end
		end,}
	}
	)
	...
	table.insert(module, {
		...
		Args = {
			...
			function(obj, value)
				...
				YARHMPointSave:set("SMRGlobal_themeCode", value)
			end
		}
	})
	...
	table.insert(module, {
		Type = "Toggle",
		Args = {"Hide SMR+/Developer tags", function(_, state)
	...
	table.insert(module, {
		Type = "Dropdown",
		Args = {"how rizz is smr", {"very rizz", "no rizz"}, function(Self, item)
			print(item)
		end,}
	})
...
local function VFKZNVN_routine() -- Script: StarterGui.SMR.Init
	local script = Instance.new("LocalScript")
	script.Name = "Init"
	script.Parent = Converted["_YARHM"]
	local req = require
	...
	local s, e = pcall(function()
		if get_hidden_gui or gethui then
			...
			--print("[SMR] - Using get_hidden_gui for anti-detection.")
		elseif (not is_sirhurt_closure) and (syn and syn.protect_gui) then
			script.Parent.Name = randomString()
			syn.protect_gui(script.Parent)
			script.Parent.Parent = COREGUI
			--print("[SMR] - Using syn.protect_gui for anti-detection.")
		elseif COREGUI:FindFirstChild('RobloxGui') then
			script.Parent.Parent = COREGUI.RobloxGui
			--print("[SMR] - Using RobloxGui for anti-detection.")
		else
			--warn("[SMR] - Using CoreGui as anti-detection. This is the most basic coverage and can still be detected.")
		end
	end)

	--print("[SMR] - SMR is now in " .. tostring(script.Parent:GetFullName()))
	if not s then
		--warn("[SMR] - Attempts at anti-detection failed. Using CoreGui as anti-detection.")
		warn(e)	
	end

	--printidentity("[SMR] - Your executor level (identity) is")

	local getExeName = identifyexecutor or getexecutorname or function() return "Yet Another Roblox Executor v1.2" end
	--print("[SMR] - Your executor is " .. getExeName())
...
local function WGUQQC_routine() -- Script: StarterGui.YARHM.Forsaken
	...
	if (module.gameId ~= game.GameId) then
		table.insert(module, {
			Type = "Text",
			Args = {"SMR now includes an official <font color='#FFFF00'>Forsaken</font> module!"}
		})
		table.insert(module, {
			Type = "Text",
...
local function ULIKGUX_routine() -- Script: StarterGui.YARHM.Murder Mystery 2
	...
	local function getPredictedPosition(player, shootOffset)
		...
		elseif predictionAIEngine and not getgenv().YARHMNetwork.predictPos then
			fu.notification("SMR AI Engine is not available. Reverting to basic prediction.")	
		end
    ...
    local function knifeThrow(silent)
        ...
        local argsThrowRemote = {
            ...
            localplayer.Character.RightHand.Position
        }

        if spawnAtPlayer then
            argsThrowRemote[2] = nearestHRP.Position + (nearestHRP.CFrame.LookVector * 5)
        end
        -- task.spawn(function()
        --     task.wait(2)
			...
			Auto_Shoot_murderer = function()
				...
				if findSheriff() == localplayer and autoShooting then
					...
					repeat
						task.wait(0.1)
						local murderer = findMurderer() or findSheriffThatsNotMe()
						if not murderer then warn("[SMR] > MM2 Autoshoot - No murderer.") continue end
						local murdererPosition = murderer.Character.HumanoidRootPart.Position
						local characterRootPart = localplayer.Character.HumanoidRootPart
...
local function EAAE_routine() -- Script: StarterGui.SMR.AddCustomModule.Cancel.LocalScript
	local script = Instance.new("LocalScript")
	script.Name = "LocalScript"
